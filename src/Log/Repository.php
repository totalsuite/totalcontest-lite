...
class Repository implements RepositoryContract {
	...
	public function create( $attributes ) {
		...
		$attributes = Arrays::parse( $attributes, [
			'date'      => TotalContest( 'datetime', [ 'now' ] ),
			'ip'        => $this->request->ip(),
		...
		if ( empty( $attributes['contest_id'] ) || empty( $attributes['submission_id'] ) || empty( $attributes['action'] ) ):
			return new \WP_Error( 'missing_fields', __( 'contest_id, submission_id and action are required' ) );
		endif;
		...
		if ( ! $inserted ):
			return new \WP_Error( 'insert_fail', __( 'Unable to insert the entry.', 'totalcontest' ) );
		endif;
	...
	public function delete( $conditions ) {
		...
		if ( empty( $where ) ):
			return new \WP_Error( 'no_conditions', __( 'No conditions were specified', 'totalcontest' ) );
		endif;
