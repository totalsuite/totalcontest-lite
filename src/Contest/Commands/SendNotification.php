...
class SendNotification extends Command {
	...
	protected function handle() {
		...
		if ( ! empty( $webhook['on']['newSubmission'] ) && ! empty( $webhook['url'] ) ):
			$this->sendWebhook( $webhook['url'], [ 'contest' => $this->contest->toArray(), 'submission' => $this->submission->toArray(), 'log' => $this->log->toArray() ] );
		endif;
		 ...
		 * @since 2.0.0
		 */
		do_action( 'totalcontest/actions/after/contest/command/notify', $this->contest, $this->submission, $this->log, $this->getTitle(), $this->getBody(), $this->getTemplate() );
