<script type="text/ng-template" id="modules-installer-component-template">
    ...
    <div class="totalcontest-modules-installer" ng-class="{uploading: $ctrl.isProcessing()}" ng-init="$root.$installer = $ctrl">
        ...
        <p><?php _e( 'If you have a template or extension in .zip format, you may install it by uploading it here.', 'totalcontest' ); ?></p>

        <button type="button" class="button button-large" ng-if="!$ctrl.file" onclick="uploadModuleFile.click()"><?php _e( 'Browse', 'totalcontest' ); ?></button>
        ...
        <button class="button button-large button-primary" ng-if="$ctrl.file" ng-click="$ctrl.install()"
                ng-disabled="$ctrl.isProcessing()"><?php _e( 'Install', 'totalcontest' ); ?> {{$ctrl.file.name}}
        </button>
    </div>
</script>