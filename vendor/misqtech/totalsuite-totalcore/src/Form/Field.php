...
abstract class Field implements \TotalContestVendors\TotalCore\Contracts\Form\Field {
		...
		foreach ( (array) $rules as $rule => $ruleArgs ):
			...
			if ( is_callable( $callback ) ):
				...
				if ( $returnedValue !== true ):
					...
					$this->errors[ $rule ] = str_replace( [ '{{label}}' ], [ $this->getOption( 'label', __( 'This field', \TotalContestVendors\TotalCore\Application::getInstance()->env( 'slug' ) ), true ) ], $returnedValue );
				...
				$errors = array_map(
					function ( $item ) {
						return str_replace(
							...
							[
								$item,
								\TotalContestVendors\TotalCore\Application::getInstance()->env( 'slug' ),
							],
							$this->errorTemplate );
					},
					$this->errors
	...
	public function getLabelHtmlElement() {
		...
		$defaults = [
			...
			'class'   => \TotalContestVendors\TotalCore\Application::getInstance()->env( 'slug' ) . '-form-field-label',
		];
		...
		if ( $label ):
			...
			$label = new \TotalContestVendors\TotalCore\Helpers\Html( 'label', $attributes, $content );
		endif;
